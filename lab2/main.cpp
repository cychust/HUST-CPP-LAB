            ...
            switch (operate) {
                ...
                case 5:
                    ...
                    if (!isInitial)printf("\n\t栈p不存在！");
                        ...
                        printf("\n\t请输入你要查找的数据下标:\n");
                        scanf("%d", &temp);
                        if (temp <= sStack->howMany()) {
                            temp = sStack->getelem(temp);
                            printf("\n\t你要查找的数据为%d\n", temp);
                        } else {
                            printf("\n\t超出范围！\n");
                        }
        ...
        for (int i = 1; i < argc; ++i) {
            ...
            if (!strcmp(argv[i], "-I")) {
                ...
                for (; (i < argc) && (argv[i][0] >= '0' && argv[i][0] <= '9'); ++i) {
                    tmp = atoi(argv[i]);
                    int stackLength = pStack->howMany();
                ...
                if (flag == 0) {
                    pStack->print();
                    if (err == 0)
                        for (int j = 0; j < pStack->howMany(); ++j) {
                            fprintf(fp, "%d  ", pStack->getelem(j));
                        }
                } else
                    break;
                i--;
            ...
            if (!strcmp(argv[i], "-A")) {
                ...
                if (i + 1 < argc && argv[i + 1][0] >= '0' && argv[i + 1][0] <= '9') {
                    ...
                    sStack->print();
                    if (err == 0)
                        for (int j = 0; j < sStack->howMany(); ++j) {
                            fprintf(fp, "%d  ", sStack->getelem(j));
                        }
                } else {
                    printf("E  ");
                    fprintf(fp, "E  ");
